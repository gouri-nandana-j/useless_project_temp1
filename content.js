...
      let acceptCount = 0;
      let currentScale = 1.0; // Initial scale of the button
      const growthFactor = 0.05; // How much bigger it gets with each click

      // Added a CSS transition for a smooth scaling animation
...
        if (acceptCount < requiredAccepts) {
          // Increase the scale and apply it to the button
          currentScale += growthFactor;
          this.style.transform = `scale(${currentScale})`;

          // Update text to show progress (optional but good UX)
          this.textContent = `Accept`;
        } else {
          this.style.transform = `scale(1)`; // Reset scale before unlocking
